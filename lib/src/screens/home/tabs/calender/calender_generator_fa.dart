...
//    currentGrid.persianTitle = "${shamsi.month}  ${shamsi.year}";
    currentGrid.setPersianTitle(format1(shamsi,nationality));
...
//      cell_item.dateTime = dayGrid;
      ...
      checkInThisMonth(cell_item,dayGrid,shamsi.month,currentGrid,i);
      ...
      if (!selectDay)
        selectDay = selectOneDay(dayGrid,shamsi.month,currentGrid,i);
...
//        print('day grdi : ${dayGrid}');
  ...
  void checkInThisMonth(CellItem item,DateTime pointDate,int monthNumber,GirdItem grid_item,int index) {
    ...
    if (dateShamsi.month != monthNumber)
...
//    cell_item.isToday = true;
  ...
  DateTime goToFirstDayGrid(DateTime dateInGrid) {
    ...
    while (true)
    {
      ...
      if (counterShamsi.day == 1)
        isAchieveFirst = true;

      if (isAchieveFirst && counterShamsi.weekDay == 1)
      {
        return counterDate;
      }
      else
