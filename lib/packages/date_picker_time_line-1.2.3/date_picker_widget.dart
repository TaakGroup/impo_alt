...
typedef DateChangeListener = void Function(Jalali selectedDate);
    ...
    return Container(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          Jalali date;
          Jalali _date = widget.startDate + index;
          date = new Jalali(_date.year, _date.month, _date.day);
...
//            print("Inside Inactive dates.");
            for (Jalali inactiveDate in widget.inactiveDates!) {
          ...
          if (widget.activeDates != null) {
            isDeactivated = true;
            isDeactivated = true;
            for (Jalali activateDate in widget.activeDates!) {
  ...
  bool _compareDate(Jalali date1, Jalali date2) {
...
class DatePickerController {
  ...
  void animateToDate(Jalali date,
      {duration = const Duration(milliseconds: 500), curve = Curves.linear}) {
    assert(_datePickerState != null,
        'DatePickerController is not attached to any DatePicker View.');
  ...
  double _calculateDateOffset(Jalali date) {
    DateTime strDate = _datePickerState!.widget.startDate.toDateTime();
    ...
    int offset = _date.difference(strDate).inDays;
